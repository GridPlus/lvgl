...
#define NORMAL_FONT            &sans_serif_20
#define HEADER_FONT          &sans_serif_20
#define HEADER_COLOR_DARK       LV_COLOR_HEX(0x292929)
#define BG_COLOR_DARK           LV_COLOR_HEX(0x414141)
#define HEADER_TEXT_COLOR_DARK  LV_COLOR_HEX(0xEAEAEA) 
#define TEXT_COLOR_DARK         LV_COLOR_HEX(0xFFFFFF)
#define SUB_TEXT_COLOR_DARK     LV_COLOR_HEX(0xEAEAEA)
/**********************
 *      TYPEDEFS
 **********************/
 ...
 *      MACROS
 **********************/
// #define GP_COLOR_FIX(x)     ((lv_color_t){{.red = (x).blue, .green = (x).green, .blue = (x).red}})
#define GP_COLOR_FIX(x) (x)
/**********************
 *   STATIC FUNCTIONS
 **********************/
...
static void basic_init(void)
{
    ...
    def.text.font = NORMAL_FONT;
    def.body.radius = DEF_RADIUS;

    static lv_style_t bg, panel;
    lv_style_copy(&bg, &def);
    bg.body.main_color = GP_COLOR_FIX(LV_COLOR_HEX(0xf0f0f0));
    ...
    // panel.body.main_color = GP_COLOR_FIX(LV_COLOR_HEX3(0xfff));
    panel.body.main_color = BG_COLOR_DARK;
    panel.body.grad_color = panel.body.main_color;
    panel.body.border.width = 1;
    // panel.body.border.color = GP_COLOR_FIX(LV_COLOR_HEX3(0xbbb));
    panel.body.border.color = BG_COLOR_DARK;
    panel.body.border.opa = LV_OPA_COVER;
    panel.body.shadow.color = GP_COLOR_FIX(DEF_SHADOW_COLOR);
    panel.body.shadow.type = LV_SHADOW_BOTTOM;
    panel.body.shadow.width = 4;
    panel.body.padding.hor = 0; //LV_DPI / 8;
    panel.body.padding.ver = 0; //LV_DPI / 8;
    panel.body.padding.inner = 0; //LV_DPI / 12;
    // panel.text.color = GP_COLOR_FIX(LV_COLOR_HEX3(0x333));
    panel.text.color = TEXT_COLOR_DARK;

    lv_style_copy(&sb, &def);
    // sb.body.main_color = GP_COLOR_FIX(LV_COLOR_BLACK);
    sb.body.main_color = GP_COLOR_FIX(LV_COLOR_WHITE);

    // sb.body.main_color = 
    sb.body.grad_color = GP_COLOR_FIX(LV_COLOR_BLACK);
...
static void btn_init(void)
{
    ...
    rel.body.main_color = GP_COLOR_FIX(LV_COLOR_HEX3(0xfff));
    ...
    // rel.body.shadow.color = GP_COLOR_FIX(DEF_SHADOW_COLOR);
    // rel.body.shadow.color = GP_COLOR_FIX(LV_COLOR_HEX3(0x0));
    // rel.body.shadow.type = LV_SHADOW_BOTTOM;
    // rel.body.shadow.width = 6;
    rel.text.color = GP_COLOR_FIX(LV_COLOR_HEX3(0x333));

    rel.body.border.width = 1;
    rel.body.border.color = GP_COLOR_FIX(LV_COLOR_HEX(0xcacaca));


    lv_style_copy(&pr, &rel);
    pr.body.main_color = GP_COLOR_FIX(lv_color_hsv_to_rgb(_hue, 90, 60));
    pr.body.grad_color = pr.body.main_color;
    pr.body.shadow.width = 4;

    lv_style_copy(&tgl_rel, &rel);
    tgl_rel.body.main_color = GP_COLOR_FIX(lv_color_hsv_to_rgb(_hue, 95, 50));
    tgl_rel.body.grad_color = tgl_rel.body.main_color;
    tgl_rel.body.shadow.width = 4;

    lv_style_copy(&tgl_pr, &tgl_rel);
    tgl_pr.body.main_color = GP_COLOR_FIX(lv_color_hsv_to_rgb(_hue, 95, 40));
    tgl_pr.body.grad_color = tgl_pr.body.main_color;
    tgl_pr.body.shadow.width = 2;

    lv_style_copy(&ina, &rel);
    ina.body.main_color = GP_COLOR_FIX(LV_COLOR_HEX3(0xccc));
    ina.body.grad_color = ina.body.main_color;
    ina.body.shadow.width = 0;
    ina.text.color = GP_COLOR_FIX(lv_color_hsv_to_rgb(_hue, 95, 5));
...
static void label_init(void)
{
    ...
    prim.text.font = NORMAL_FONT;
    prim.text.color = GP_COLOR_FIX(lv_color_hsv_to_rgb(_hue, 80, 10));

    lv_style_copy(&sec, &prim);
    sec.text.color = GP_COLOR_FIX(lv_color_hsv_to_rgb(_hue, 80, 75));

    lv_style_copy(&hint, &prim);
    hint.text.color = GP_COLOR_FIX(lv_color_hsv_to_rgb(_hue, 40, 90));
...
static void img_init(void)
{
    ...
    img_light.image.color = GP_COLOR_FIX(lv_color_hsv_to_rgb(_hue, 15, 85));
    img_light.image.intense = LV_OPA_80;

    lv_style_copy(&img_dark, &def);
    img_light.image.color = GP_COLOR_FIX(lv_color_hsv_to_rgb(_hue, 85, 65));
    img_light.image.intense = LV_OPA_80;
...
static void led_init(void)
{
    ...
    led.body.main_color = GP_COLOR_FIX(lv_color_hsv_to_rgb(_hue, 100, 100));
    led.body.grad_color = GP_COLOR_FIX(lv_color_hsv_to_rgb(_hue, 100, 100));
    led.body.border.color = GP_COLOR_FIX(lv_color_hsv_to_rgb(_hue, 60, 60));
    led.body.shadow.color = GP_COLOR_FIX(lv_color_hsv_to_rgb(_hue, 100, 100));
...
static void bar_init(void)
{
    ...
    bar_bg.body.main_color = GP_COLOR_FIX(lv_color_hsv_to_rgb(_hue, 15, 95));
    ...
    bar_indic.body.main_color = GP_COLOR_FIX(lv_color_hsv_to_rgb(_hue, 85, 70));
...
static void sw_init(void)
{
    ...
    sw_knob_on.body.shadow.color = GP_COLOR_FIX(DEF_SHADOW_COLOR);


    lv_style_copy(&sw_knob_off, &sw_knob_on);
    sw_knob_off.body.main_color = GP_COLOR_FIX(LV_COLOR_HEX(0xfafafa));
    sw_knob_off.body.grad_color = sw_knob_off.body.main_color;
    sw_knob_off.body.border.width = 1;
    sw_knob_off.body.border.color = GP_COLOR_FIX(LV_COLOR_HEX3(0x999));
...
static void lmeter_init(void)
{
    ...
    lmeter.body.main_color = GP_COLOR_FIX(lv_color_hsv_to_rgb(_hue, 75, 90));
    lmeter.body.grad_color = lmeter.body.main_color;
    lmeter.body.padding.hor = LV_DPI / 10;                       /*Scale line length*/
    lmeter.line.color = GP_COLOR_FIX(LV_COLOR_HEX3(0x999));
    lmeter.line.width = 2;
...
static void gauge_init(void)
{
    ...
    gauge.body.main_color = GP_COLOR_FIX(lv_color_hsv_to_rgb(_hue, 10, 60));
    ...
    gauge.body.border.color = GP_COLOR_FIX(LV_COLOR_HEX3(0x999));
    gauge.text.color = GP_COLOR_FIX(LV_COLOR_HEX3(0x333));
    gauge.line.width = 3;
    gauge.line.color = GP_COLOR_FIX(lv_color_hsv_to_rgb(_hue, 95, 70));
...
static void arc_init(void)
{
    ...
    arc.line.width = 10;
    arc.line.color = GP_COLOR_FIX(lv_color_hsv_to_rgb(_hue, 90, 90));

    /*For prelaoder*/
    arc.body.border.width = HOR_PADDING;
    arc.body.border.color = GP_COLOR_FIX(lv_color_hsv_to_rgb(_hue, 30, 90));
...
static void calendar_init(void)
{
    ...
    ina_days.text.color = GP_COLOR_FIX(lv_color_hsv_to_rgb(_hue, 0, 70));

    static lv_style_t high_days;
    lv_style_copy(&high_days, &def);
    high_days.text.color = GP_COLOR_FIX(lv_color_hsv_to_rgb(_hue, 80, 90));

    static lv_style_t week_box;
    lv_style_copy(&week_box, &def);
    week_box.body.main_color = GP_COLOR_FIX(lv_color_hsv_to_rgb(_hue, 40, 100));
    week_box.body.grad_color = GP_COLOR_FIX(lv_color_hsv_to_rgb(_hue, 40, 100));
    ...
    today_box.body.main_color = GP_COLOR_FIX(LV_COLOR_WHITE);
    today_box.body.grad_color = GP_COLOR_FIX(LV_COLOR_WHITE);
...
static void cb_init(void)
{
    ...
    lv_style_copy(&pr, &rel);
    pr.body.main_color = GP_COLOR_FIX(LV_COLOR_HEX3(0xccc));
    pr.body.grad_color = pr.body.main_color;
    pr.body.shadow.width = 0;

    lv_style_copy(&tgl_rel, &rel);
    tgl_rel.body.main_color = GP_COLOR_FIX(lv_color_hsv_to_rgb(_hue, 75, 85));
    tgl_rel.body.grad_color = tgl_rel.body.main_color;
    tgl_rel.body.shadow.type = LV_SHADOW_FULL;
    tgl_rel.body.shadow.width = 0;

    lv_style_copy(&tgl_pr, &tgl_rel);
    tgl_pr.body.main_color = GP_COLOR_FIX(lv_color_hsv_to_rgb(_hue, 75, 65));
    tgl_pr.body.grad_color = tgl_pr.body.main_color;
...
static void btnm_init(void)
{
    ...
    bg.text.color = GP_COLOR_FIX(LV_COLOR_HEX3(0x555));

    lv_style_copy(&rel, theme.panel);
    /*rel.body.border.width = 10;
    rel.body.border.color = GP_COLOR_FIX(LV_COLOR_HEX3(0x0));
    rel.body.shadow.width = 0;
*/
    lv_style_copy(&pr, &rel);
    pr.glass = 0;
    pr.body.main_color = GP_COLOR_FIX(LV_COLOR_HEX3(0xddd));
    pr.body.grad_color = pr.body.main_color;
    pr.body.border.width = 0;
    pr.body.empty = 0;

    lv_style_copy(&tgl_rel, &pr);
    tgl_rel.body.main_color = GP_COLOR_FIX(lv_color_hsv_to_rgb(_hue, 90, 70));
    tgl_rel.body.grad_color = tgl_rel.body.main_color;
    tgl_rel.text.color = GP_COLOR_FIX(lv_color_hsv_to_rgb(_hue, 5, 95));


    lv_style_copy(&tgl_pr, &tgl_rel);
    tgl_pr.body.main_color = GP_COLOR_FIX(lv_color_hsv_to_rgb(_hue, 95, 65));
    tgl_pr.body.grad_color = tgl_pr.body.main_color;
...
static void kb_init(void)
{
    ...
    theme.kb.btn.rel = theme.btnm.btn.rel;
...
static void mbox_init(void)
{
    ...
    rel.glass = 0;
    rel.text.font = NORMAL_FONT;
    rel.text.color = GP_COLOR_FIX(lv_color_hsv_to_rgb(_hue, 85, 75));

    lv_style_copy(&pr, theme.btnm.btn.pr);
    pr.text.color = GP_COLOR_FIX(lv_color_hsv_to_rgb(_hue, 85, 60));
...
static void ta_init(void)
{
    ...
    oneline.body.border.color = GP_COLOR_FIX(LV_COLOR_HEX3(0x333));
    oneline.body.border.opa = LV_OPA_COVER;
    oneline.text.color = GP_COLOR_FIX(LV_COLOR_HEX3(0x333));

    theme.ta.area = theme.panel;
    theme.ta.area->body.shadow.width = 0;
...
static void list_init(void)
{
    ...
    // list_bg.body.main_color = GP_COLOR_FIX(LV_COLOR_HEX3(0xfff));
    list_bg.body.main_color = BG_COLOR_DARK;
    list_bg.body.grad_color = list_bg.body.main_color;
    list_bg.body.border.color = list_bg.body.main_color;
    list_bg.body.shadow.color = list_bg.body.main_color;
    list_bg.text.font = NORMAL_FONT;
    list_bg.text.color = SUB_TEXT_COLOR_DARK;
    ...
    pr.glass = 0;
    // pr.body.main_color = GP_COLOR_FIX(LV_COLOR_HEX3(0xddd));
    pr.body.main_color = HEADER_TEXT_COLOR_DARK;
    pr.body.grad_color = pr.body.main_color;
    pr.body.border.width = 0;
    pr.body.empty = 0;
    pr.body.radius = 0;
    // pr.text.font = _font;
    pr.text.font = NORMAL_FONT;

    lv_style_copy(&tgl_rel, &pr);
    tgl_rel.body.main_color = GP_COLOR_FIX(lv_color_hsv_to_rgb(_hue, 90, 70));
    tgl_rel.body.grad_color = tgl_rel.body.main_color;
    tgl_rel.text.color = GP_COLOR_FIX(lv_color_hsv_to_rgb(_hue, 5, 95));
    tgl_rel.text.font = NORMAL_FONT;

    lv_style_copy(&tgl_pr, &tgl_rel);
    tgl_pr.body.main_color = GP_COLOR_FIX(lv_color_hsv_to_rgb(_hue, 90, 60));
    tgl_pr.body.grad_color = tgl_pr.body.main_color;
...
static void ddlist_init(void)
{
    ...
    sel.body.main_color = GP_COLOR_FIX(lv_color_hsv_to_rgb(_hue, 90, 70));
    sel.body.grad_color = sel.body.main_color;
    sel.body.border.width = 0;
    sel.body.shadow.width = 0;
    sel.text.color = GP_COLOR_FIX(lv_color_hsv_to_rgb(_hue, 5, 95));
...
static void roller_init(void)
{
    ...
    roller_bg.text.font = NORMAL_FONT;
    roller_bg.glass = 0;

    lv_style_copy(&roller_sel, &roller_bg);
    roller_sel.text.color = GP_COLOR_FIX(lv_color_hsv_to_rgb(_hue, 90, 70));
...
static void tabview_init(void)
{
    ...
    indic.body.main_color = GP_COLOR_FIX(lv_color_hsv_to_rgb(_hue, 90, 70));
    ...
    pr.body.main_color = GP_COLOR_FIX(LV_COLOR_HEX3(0xbbb));
    pr.body.grad_color = pr.body.main_color;
    pr.body.border.width = 0;
    pr.body.empty = 0;
    pr.body.radius = 0;
    pr.body.border.width = 1;
    pr.body.border.color = GP_COLOR_FIX(LV_COLOR_HEX3(0x888));
    pr.body.border.part = LV_BORDER_BOTTOM;
    pr.body.border.opa = LV_OPA_COVER;
    pr.text.color = GP_COLOR_FIX(LV_COLOR_HEX3(0x111));

    lv_style_copy(&tgl_rel, &lv_style_transp);
    tgl_rel.glass = 0;
    tgl_rel.text.font = NORMAL_FONT;
    tgl_rel.text.color = GP_COLOR_FIX(lv_color_hsv_to_rgb(_hue, 90, 70));

    lv_style_copy(&tgl_pr, &def);
    tgl_pr.body.main_color = GP_COLOR_FIX(lv_color_hsv_to_rgb(_hue, 15, 85));
    tgl_pr.body.grad_color = tgl_pr.body.main_color;
    tgl_pr.body.border.width = 0;
    tgl_pr.body.empty = 0;
    tgl_pr.body.radius = 0;
    tgl_pr.text.color = GP_COLOR_FIX(lv_color_hsv_to_rgb(_hue, 90, 60));
...
static void win_init(void)
{
    ...
    // header.body.main_color = GP_COLOR_FIX(LV_COLOR_HEX(0xf2f2f2));
    header.body.main_color = HEADER_COLOR_DARK;
    header.body.grad_color = header.body.main_color;
    header.body.radius = 0;
    header.body.border.width = 2;
    // header.body.border.color = GP_COLOR_FIX(LV_COLOR_HEX(0xcacaca));
    header.body.border.color = HEADER_COLOR_DARK;
    ...
    // header.text.color = GP_COLOR_FIX(LV_COLOR_HEX(0x6c6c6c));
    header.text.color = HEADER_TEXT_COLOR_DARK;
    header.text.line_space = 0;
    header.text.font = HEADER_FONT;

    lv_style_copy(&pr, &def);
    // pr.body.main_color = GP_COLOR_FIX(LV_COLOR_HEX3(0x0));
    ...
    pr.body.radius = 0;
    // pr.text.color = GP_COLOR_FIX(LV_COLOR_HEX3(0x111));
    pr.text.color = TEXT_COLOR_DARK;
    static lv_style_t bg;
    lv_style_copy(&bg, &lv_style_transp);
    bg.body.main_color = GP_COLOR_FIX(LV_COLOR_HEX3(0xccc));
    // bg.body.main_color = BG_COLOR_DARK;
    bg.body.padding.inner = 0;
    bg.body.padding.ver = -15;
    bg.body.padding.hor = 15;

    theme.win.bg = theme.panel;
    theme.win.sb = &sb;
    theme.win.header = &header;
    theme.win.content.bg = &bg;
